...
df <- read.csv(file="pooled_inter_day.csv", header=TRUE, sep=",") %>%
  as.data.frame() %>%
  filter(abs(L) <= width) 
...
#new starting value for constrained optimization
x0<- coef 
...
# Iteration with new starting value
diff = 1
iteration = 0
while (diff >= tol) {
...
fitting_plot
ggsave(file = "fig3_kernel_daily.pdf", plot = fitting_plot)

beta_matrix <- rbind(filter(df_plot, L<=0),filter(dff_plot, L>=0)) %>%
  select(-se, -min, -max, -w) %>%
write.csv("fitted_kernel_daily.csv")


#rescale to get probability density
scalar <- sum(df_plot$beta)
scaled <- rbind(filter(df_plot, L<=0),filter(dff_plot, L>=0)) %>%
  select(-se, -min, -max, -w, -pred) %>%
  cbind(rbind(select(filter(DF, L<=0),pred),select(filter(DFF, L>=0),pred)))/ scalar 
...
ggsave(file = "../fig3_kernel_daily_scaled.pdf", plot = scaled_plot)
