...
df <- read.csv(file="pooled_inter_year.csv", header=TRUE, sep=",") %>%
  as.data.frame() %>%
  filter(abs(L) <= width) 
...
fitting_plot
ggsave(file = "fig3_kernel_yearly.pdf", plot = fitting_plot)

beta_matrix <- rbind(filter(df_plot, L<=0),filter(dff_plot, L>=0)) %>%
  select(-se, -min, -max, -w) %>%
write.csv("fitted_kernel_yearly.csv")
...
ggsave(file = "../fig3_kernel_yearly_scaled.pdf", plot = scaled_plot)